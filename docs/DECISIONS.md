...
| Execution v0.1 | Tree-walking interpreter | Fastest path to `neko run` |
...
| Error handling | `try` / `catch`, `throw`, typed `error` values, `neko_errors` crate | Structured errors with E#### codes; see [ERRORS.md](ERRORS.md) |
...
```
Source (.neko) → Lexer → Parser → AST → [Interpreter | IR → Bytecode → VM]
```

## Module System

- `import "path"` resolves relative to the importing file's directory
- Each `.neko` file is a module exporting top-level `fn` definitions

## Project Layout

- `neko.config` — project manifest (name, version, entry)
- `src/` — source files
- `tests/` — test programs run by `neko test`
...
exposing a clean Neko API, wrapping battle-tested libraries (axum, sqlx, serde_json, ureq, etc.).
...
- `neko run` and `neko build` write `.nekobc` files under `.neko-build/` (cwd), keyed by source path
- Caches invalidate on source mtime, `BYTECODE_CACHE_VERSION`, and builtin fingerprint changes
- Atomic write: `*.nekobc.tmp` then rename
