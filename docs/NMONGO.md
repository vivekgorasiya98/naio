...
Fast MongoDB for Neko programs: connection pooling, CRUD, aggregation, indexes, transactions, GridFS, change streams, and async background tasks. Implemented in Rust via the official `mongodb` driver.
...
MongoDB operators (`$match`, `$set`, `$gte`, …) cannot be used as bare object keys in Neko source (`{$match: ...}` is a parse error). Use `json.parse` for those documents:
...
| `to_extended_json(value)` | Neko value → extended JSON string |
| `from_extended_json(s)` | Parse extended JSON → Neko value |
...
- MongoDB server reachable from the host running `neko`
- For integration tests: set `NEKO_MONGO_URL` or `MONGO_URL` (e.g. `mongodb://localhost:27017`)
