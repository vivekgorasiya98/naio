...
use crate::{call_niao_function, niao_call_hook_active, ValueRef};
use niao_ast::Span;
...
pub fn poll_one(span: Span) -> bool {
    ...
    let result = if niao_call_hook_active() {
        match call_niao_function(job.callee, &args, span) {
