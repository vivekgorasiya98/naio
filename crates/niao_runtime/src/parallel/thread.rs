//! OS thread spawn/join for Niao callbacks.
...
fn spawn_niao_thread(
    ...
    let handle = thread::spawn(move || {
        ...
        let args: Vec<ValueRef> = sendable_args
            ...
            .collect();
        let result = match call_niao_function(callee, &args, span) {
...
pub fn parallel_thread_spawn(args: &[ValueRef], span: Span) -> ParallelResult {
    ...
    let thread = spawn_niao_thread(callee, sendable_args, span);
