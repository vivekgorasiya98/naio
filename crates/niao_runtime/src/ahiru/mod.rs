...
pub use app::{
    ...
    mount_native_ping, queue_pending_serve, response_from_niao, start_pending_server, AppHandle,
};
...
fn ahiru_app_new(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_app_from_config(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    ...
    if args.len() == 4 {
        if let Value::Object(map) = &*args[3].borrow() {
            if let Some(schema) = map.get("schema") {
                meta.schema = Some(app::make_niao_handler(schema.clone(), span));
            }
            if let Some(guard) = map.get("auth").or_else(|| map.get("guard")) {
                if matches!(&*guard.borrow(), Value::Function(_)) {
                    meta.guard = Some(app::make_niao_handler(guard.clone(), span));
                }
            }
        }
    }
...
fn ahiru_app_get(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    register_route(args, span, ahiru_core::HttpMethod::Get, "ahiru_app_get")
}

fn ahiru_app_post(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    register_route(args, span, ahiru_core::HttpMethod::Post, "ahiru_app_post")
}

fn ahiru_app_put(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_app_ws(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    ...
    let ws_fn = app::make_niao_ws_handler(handler, span);
...
fn ahiru_app_listen(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_app_routes(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_response(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_sse_start(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_sse_write(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_json_response(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_db_exec(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_app_set_logging(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_app_init_db(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_native_routes(_args: &[ValueRef], _span: Span) -> NiaoResult<ValueRef> {
    Ok(Value::Bool(options::native_routes_enabled()).ref_cell())
}

fn ahiru_native_mount_health(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn ahiru_native_mount_ping(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
