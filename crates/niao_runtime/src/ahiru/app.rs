//! App handle table and Niao handler bridge.
...
fn resolve_handler(handler_id: u64) -> Result<(ValueRef, Span), String> {
    ...
    let handler = resolve_niao_function_by_name(&name)
...
pub fn response_from_niao(val: &Value) -> Result<AhiruResponse, String> {
...
fn invoke_sync(handler_id: u64, mut ctx: RequestContext, quiet: bool) -> Result<AhiruResponse, String> {
    ...
    let result = (|| {
        let req = ctx_bridge::ctx_to_niao(&mut ctx);
        let result = call_niao_function(handler, &[req], span).map_err(|e| e.to_string())?;
        let borrowed = result.borrow();
        niao_to_response(&borrowed)
    })();
    if quiet {
...
pub fn make_niao_handler(handler: ValueRef, span: Span) -> HandlerFn {
...
pub fn make_niao_ws_handler(handler: ValueRef, span: Span) -> WsHandlerFn {
    let handler_id = with_handlers_mut(|r| r.register(handler, span));
    Arc::new(move |ctx, sink| {
        ...
        Box::pin(async move {
            if let Ok((handler, span)) = resolve_handler(handler_id) {
                let mut ctx_mut = ctx;
                let req = ctx_bridge::ctx_to_niao(&mut ctx_mut);
                let _ = call_niao_function(handler, &[req], span);
            }
...
fn niao_to_response(val: &Value) -> Result<AhiruResponse, String> {
...
fn pool_invoke(handler_id: u64, fields: HashMap<String, String>) -> Result<AhiruResponse, String> {
    let (handler, span) = resolve_handler(handler_id)?;
    let req = ctx_bridge::fields_to_niao(&fields);
    let result = call_niao_function(handler, &[req], span).map_err(|e| e.to_string())?;
    let borrowed = result.borrow();
    niao_to_response(&borrowed)
}
