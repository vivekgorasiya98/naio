...
use crate::nsqlite::types::{apply_params, niao_to_bound, BoundValue};
...
pub fn from_sqlite(conn_id: u64, sql: &str, params: &[crate::ValueRef], span: Span) -> Result<DataFrame, String> {
    handles::with_conn_mut(conn_id, "ncl_from_sqlite", span, |handle| {
        let bound: Result<Vec<BoundValue>, _> = params
            .iter()
            .map(|p| niao_to_bound(&p.borrow(), span))
            .collect();
...
impl ColumnBuilder {
    ...
    fn push_sql(&mut self, val: SqlValue) {
        ...
        match self {
            ...
            ColumnBuilder::Any(v) => {
                let nv = crate::nsqlite::types::sql_to_niao(val);
                v.push(nv.ref_cell());
            }
        }
    }
    ...
    if let crate::Value::Array(rows) = &*rows_ref.borrow() {
        for row_ref in rows {
            ...
            for (i, cell) in cells.iter().enumerate() {
                ...
                push_niao_value(&mut builders[i], &cell.borrow());
            }
        }
    }
...
fn push_niao_value(builder: &mut ColumnBuilder, val: &crate::Value) {
    match val {
        crate::Value::Int(n) => {
