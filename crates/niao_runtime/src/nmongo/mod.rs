...
use bg::{
    nmongo_async_bulk_write, nmongo_async_count_documents, nmongo_async_find,
    nmongo_parallel_count_documents, nmongo_task_cancel, nmongo_task_done, nmongo_task_result,
    nmongo_task_results, nmongo_task_wait, nmongo_task_wait_all,
};
...
use crud::{
    ...
    nmongo_drop_collection, nmongo_find, nmongo_find_one, nmongo_insert_many,
    nmongo_insert_many_chunks, nmongo_insert_one, nmongo_list_collections, nmongo_replace_one,
    nmongo_update_many, nmongo_update_one,
};
use gridfs::{
...
fn nmongo_object_id(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    ...
    Ok(self::types::bson_to_niao(bson).ref_cell())
}

fn nmongo_is_object_id(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn nmongo_to_extended_json(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    arity(args, 1, "nmongo_to_extended_json", span)?;
    let s = to_extended_json(&args[0], span)?;
    Ok(Value::String(s).ref_cell())
}

fn nmongo_from_extended_json(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
fn all_builtins() -> Vec<(&'static str, NativeFn)> {
    vec![
        ...
        ("nmongo_task_wait_all", Rc::new(nmongo_task_wait_all)),
        ("nmongo_task_result", Rc::new(nmongo_task_result)),
        ("nmongo_task_results", Rc::new(nmongo_task_results)),
...
pub fn namespace() -> Value {
    ...
    bind(&mut map, "task_results", Rc::new(nmongo_task_results));
