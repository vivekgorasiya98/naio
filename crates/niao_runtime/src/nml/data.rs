//! Data pipeline builtins (niao_data bridge).
...
pub fn nml_from_dataframe(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    ...
    ncl_with(df_id, "nml_from_dataframe", span, |h| {
        ...
        let (x, y) = niao_data::dataframe_columns_to_tensors(&feat_cols, &labels)
...
pub fn nml_train_test_split(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
pub fn nml_normalize(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
pub fn nml_standardize(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
pub fn nml_one_hot(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
pub fn nml_batch(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    if args.len() == 3 {
...
pub fn ncl_to_nml_matrix(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    ...
    ncl_with(id, "ncl_to_nml_matrix", span, |h| match h {
        NclHandle::Series(s) => {
            ...
            let n = col.len();
            let t = niao_tensor::Tensor::from_cpu_data(&[n, 1], col, niao_tensor::Device::Cpu)
                .map_err(|e| e.to_string())?;
...
pub fn npg_to_ncl(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
#[cfg(feature = "nmongo")]
pub fn nmongo_to_ncl(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
pub fn nmongo_to_ncl(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    ...
    Err(RuntimeError::at(
        ...
        "nmongo_to_ncl requires niao build with nmongo feature",
    ))
}

pub fn nml_node_features_from_ncl(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
pub fn nml_pipeline(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
...
pub fn nml_columnar_epoch(args: &[ValueRef], span: Span) -> NiaoResult<ValueRef> {
    ...
    let mut epoch = niao_data::ColumnarEpoch::new(feat_cols, label_col, batch);
