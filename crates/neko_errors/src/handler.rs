...
use crate::neko_error::NekoError;
use std::fmt;
use std::io::{self, Write};

/// Formats and reports Neko errors consistently across CLI and tooling.
...
impl ErrorHandler {
    ...
    pub fn format_error(&self, err: &NekoError) -> String {
        self.format_diagnostic(&err.diagnostic())
    }

    pub fn report(&self, err: &NekoError) -> io::Result<()> {
...
mod tests {
    ...
    fn formats_runtime_error() {
        ...
        let err = NekoError::Runtime(RuntimeError::division_by_zero(Span::dummy()));
