...
use gc::MemoCache;
use neko_ast::{BinOp, Span, UnaryOp};
use neko_bytecode::{BytecodeFunction, BytecodeModule, FastPath, OpCode};
use neko_runtime::*;
...
impl Vm {
    ...
    pub fn run(&mut self, module: &BytecodeModule, base_dir: &Path) -> Result<(), VmError> {
        ...
        neko_runtime::mem::set_vm_arena_reader(Some(Rc::new(move || {
        ...
        neko_runtime::mem::set_vm_arena_reader(None);
        result
    }
    ...
    fn run_loaded(&mut self, module: &BytecodeModule, _base_dir: &Path) -> Result<(), VmError> {
        ...
        let entry_idx = self
            .functions
            .iter()
            .position(|f| f.name == neko_ir::TOPLEVEL_FN)
        ...
        match op {
                ...
                OpCode::Throw => {
                    ...
                    let runtime_err = match &*value_ref.borrow() {
                        ...
                        other => RuntimeError::thrown(neko_errors::NekoErrorValue::from_message(
                            other.to_string(),
                            Span::dummy(),
                        )),
                    };
    ...
    fn do_make_instance(&mut self, class_idx: u16, field_count: u16) -> Result<(), VmError> {
        ...
        let field_defs: Vec<String> = class_def
            ...
            .filter_map(|m| match m {
                neko_ast::ClassMember::Field { name, .. } => Some(name.clone()),
                _ => None,
            })
            .collect();
...
mod tests {
    ...
    fn vm_runs_factorial() {
        let src = include_str!("../../../examples/factorial.neko");
        let program = parse(src).unwrap();
        let bytecode = compile_to_bytecode(&program).unwrap();
        neko_runtime::set_quiet_output(true);
        let mut vm = Vm::new();
        vm.run(&bytecode, Path::new(".")).unwrap();
        neko_runtime::set_quiet_output(false);
    }
    ...
    fn vm_runs_math_stress() {
        let src = include_str!("../../../examples/math_stress.neko");
        let program = parse(src).unwrap();
        let bytecode = compile_to_bytecode(&program).unwrap();
        neko_runtime::set_quiet_output(true);
        let mut vm = Vm::new();
    ...
    fn vm_runs_sort_100k() {
        let src = include_str!("../../../examples/sort_100k.neko");
        let program = parse(src).unwrap();
        let bytecode = compile_to_bytecode(&program).unwrap();
        neko_runtime::set_quiet_output(true);
        let mut vm = Vm::new();
    ...
    #[test]
    fn vm_runs_dsa_demo() {
        let src = include_str!("../../../examples/dsa_demo.neko");
        let program = parse(src).unwrap();
        let bytecode = compile_to_bytecode(&program).unwrap();
        neko_runtime::set_quiet_output(true);
        let mut vm = Vm::new();
    ...
    fn vm_runs_gc_heap_fixture() {
        let src = include_str!("../../../tests/gc_heap.neko");
        let program = parse(src).unwrap();
        let bytecode = compile_to_bytecode(&program).unwrap();
        neko_runtime::set_quiet_output(true);
        let mut vm = Vm::new();
