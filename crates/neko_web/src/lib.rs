...
pub enum WebError {
    ...
    Io(std::io::Error),
    Runtime(neko_interpreter::InterpreterError),
    NoRoutes,
}
...
impl From<neko_parser::ParseError> for WebError {
...
impl From<neko_interpreter::InterpreterError> for WebError {
    fn from(e: neko_interpreter::InterpreterError) -> Self {
        WebError::Runtime(e)
    }
}
...
pub async fn serve_web(file: &Path, default_port: u16) -> Result<(), WebError> {
    ...
    println!("Neko server listening on http://localhost:{port}");
