...
use crate::value::NiaoErrorValue;
use niao_ast::Span;
use std::fmt;
...
impl RuntimeError {
    ...
    pub fn to_niao_error_value(&self) -> NiaoErrorValue {
        match self {
            RuntimeError::Thrown(v) => v.clone(),
            _ => NiaoErrorValue::from_diagnostic(&self.diagnostic()),
        }
    }
    ...
    pub fn thrown(value: NiaoErrorValue) -> Self {
...
pub type NiaoResult<T> = Result<T, RuntimeError>;
