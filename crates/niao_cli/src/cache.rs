...
/// Summary of a bytecode cache cleanup pass.
...
/// Remove stale `.niaobc` entries and trim to the `keep` most recently used files.
pub fn clean_bytecode_cache(cache_dir: &Path, keep: usize, all: bool) -> CleanReport {
    ...
    if all {
        match fs::remove_dir_all(cache_dir) {
            ...
            Err(e) => eprintln!("warning: failed to remove {}: {e}", cache_dir.display()),
        }
        return report;
    }
    ...
    let mut fresh: Vec<(PathBuf, std::time::SystemTime, u64)> = Vec::new();
    for entry in entries.flatten() {
        let path = entry.path();
        let name = path.file_name().and_then(|n| n.to_str()).unwrap_or("");
        if name.ends_with(".niaobc.tmp") {
        ...
        if path.extension().and_then(|e| e.to_str()) != Some("niaobc") {
            continue;
        }
        let size = entry.metadata().map(|m| m.len()).unwrap_or(0);
        if try_load_cache(&path).is_none() {
            if fs::remove_file(&path).is_ok() {
                report.removed_files += 1;
                report.freed_bytes += size;
            }
        ...
        fresh.push((path, modified, size));
    }

    fresh.sort_by(|a, b| b.1.cmp(&a.1));
    let total_fresh = fresh.len();
    for (path, _, size) in fresh.into_iter().skip(keep) {
        if fs::remove_file(&path).is_ok() {
            report.removed_files += 1;
            report.freed_bytes += size;
        }
    }
    report.kept_files = total_fresh.min(keep);
    report
}
...
mod tests {
    ...
    #[test]
    fn cache_written_under_niao_build() {
        let _lock = CWD_LOCK.lock().unwrap();
        let tmp = std::env::temp_dir().join(format!("niao_cache_test_{}", std::process::id()));
        ...
        write_fixture(
            &tmp,
            "examples/factorial.niao",
            r#"fn main() {
    print(1)
}
"#,
        );
        let source = Path::new("examples/factorial.niao");
    ...
    fn cache_recompiles_when_source_is_newer() {
        let _lock = CWD_LOCK.lock().unwrap();
        let tmp = std::env::temp_dir().join(format!("niao_cache_fresh_{}", std::process::id()));
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();
        let prev = std::env::current_dir().unwrap();
        std::env::set_current_dir(&tmp).unwrap();

        write_fixture(&tmp, "hello.niao", "fn main() { print(1) }\n");
        let source = Path::new("hello.niao");
        let cache_dir = default_cache_dir();
    ...
    fn distinct_cache_paths_for_same_stem() {
        let _lock = CWD_LOCK.lock().unwrap();
        let tmp = std::env::temp_dir().join(format!("niao_cache_stem_{}", std::process::id()));
        let _ = fs::remove_dir_all(&tmp);
        fs::create_dir_all(&tmp).unwrap();
        let prev = std::env::current_dir().unwrap();
        std::env::set_current_dir(&tmp).unwrap();

        write_fixture(&tmp, "a/foo.niao", "fn main() { print(1) }\n");
        write_fixture(&tmp, "b/foo.niao", "fn main() { print(2) }\n");
        let cache_dir = default_cache_dir();
