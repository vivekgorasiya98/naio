//! `niao ahiru add <feature>` — bolt features onto existing projects.
...
pub fn run_add(project: &Path, feature: &str) -> Result<(), Box<dyn std::error::Error>> {
    match feature {
        "auth" => write_snippet(project, "src/routes/auth.niao", AUTH_SNIPPET)?,
        "db" => {
            write_snippet(project, "migrations/001_init.sql", DB_MIGRATION)?;
            write_snippet(project, "src/routes/db.niao", DB_SNIPPET)?;
        }
        "websocket" => write_snippet(project, "src/routes/ws.niao", WS_SNIPPET)?,
        "cache" => write_snippet(project, "src/routes/cache.niao", CACHE_SNIPPET)?,
        other => {
...
const AUTH_SNIPPET: &str = r#"// Auth routes — import from main.niao
fn mount_auth(app) {
