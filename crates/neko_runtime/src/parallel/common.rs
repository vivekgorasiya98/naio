...
pub(crate) fn arity_min(args: &[ValueRef], min: usize, name: &str, span: Span) -> NekoResult<()> {
    if args.len() < min {
        return Err(RuntimeError::at(
            span,
            codes::E1500_PARALLEL_ARITY,
...
pub(crate) fn function_arg(args: &[ValueRef], idx: usize, name: &str, span: Span) -> NekoResult<ValueRef> {
...
pub type ParallelResult = NekoResult<ValueRef>;
