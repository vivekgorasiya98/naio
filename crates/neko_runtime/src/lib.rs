use neko_ast::{BinOp, ClassDef, FnDef, Span, StructDef, TraitDef, UnaryOp};
...
pub use neko_errors::{codes, NekoErrorValue, NekoResult, RuntimeError};
pub use json::{json_parse, json_stringify};
pub use io::{io_read_file, io_write_file};

/// Hook for native code to invoke Neko functions (HTTP handlers, parallel workers).
pub type NekoCallHook = Arc<dyn Fn(ValueRef, &[ValueRef], Span) -> NekoResult<ValueRef> + Send + Sync>;

/// Optional VM-backed hook used when the interpreter is not active.
pub type NekoVmCallHook = Arc<dyn Fn(ValueRef, &[ValueRef], Span) -> NekoResult<ValueRef> + Send + Sync>;

/// Resolve a top-level Neko function by name (interpreter serve / worker threads).
pub type NekoFnResolver = Arc<dyn Fn(&str) -> Option<ValueRef> + Send + Sync>;

static GLOBAL_NEKO_CALL_HOOK: OnceLock<Mutex<Option<NekoCallHook>>> = OnceLock::new();
static GLOBAL_VM_CALL_HOOK: OnceLock<Mutex<Option<NekoVmCallHook>>> = OnceLock::new();
static GLOBAL_FN_RESOLVER: OnceLock<Mutex<Option<NekoFnResolver>>> = OnceLock::new();
static INTERPRETER_GIL: OnceLock<Mutex<()>> = OnceLock::new();

fn global_call_hook_slot() -> &'static Mutex<Option<NekoCallHook>> {
    GLOBAL_NEKO_CALL_HOOK.get_or_init(|| Mutex::new(None))
}

/// Global interpreter lock — only one Neko function may execute at a time.
pub fn interpreter_gil() -> &'static Mutex<()> {
    INTERPRETER_GIL.get_or_init(|| Mutex::new(()))
}

fn global_vm_call_hook_slot() -> &'static Mutex<Option<NekoVmCallHook>> {
    GLOBAL_VM_CALL_HOOK.get_or_init(|| Mutex::new(None))
}

/// Register a VM-backed callback for native→Neko handler dispatch.
pub fn set_neko_vm_call_hook(hook: Option<NekoVmCallHook>) {
    *global_vm_call_hook_slot().lock().unwrap() = hook;
}

pub fn neko_vm_call_hook_active() -> bool {
    global_vm_call_hook_slot().lock().unwrap().is_some()
}

/// Register a callback used by native modules to call Neko functions.
pub fn set_neko_call_hook(hook: Option<NekoCallHook>) {
    *global_call_hook_slot().lock().unwrap() = hook;
}

/// Whether a Neko call hook is registered (interpreter mode).
pub fn neko_call_hook_active() -> bool {
    global_call_hook_slot().lock().unwrap().is_some()
}

fn global_fn_resolver_slot() -> &'static Mutex<Option<NekoFnResolver>> {
    GLOBAL_FN_RESOLVER.get_or_init(|| Mutex::new(None))
}

pub fn set_neko_fn_resolver(resolver: Option<NekoFnResolver>) {
    *global_fn_resolver_slot().lock().unwrap() = resolver;
}

pub fn resolve_neko_function_by_name(name: &str) -> Option<ValueRef> {
...
/// Invoke a Neko function through the registered interpreter or VM hook.
/// Interpreter path holds the global GIL; VM path does not.
pub fn call_neko_function(
    callee: ValueRef,
    args: &[ValueRef],
    span: Span,
) -> NekoResult<ValueRef> {
    ...
    let vm_hook = vm_hook.as_ref().ok_or_else(|| {
        RuntimeError::at(
            ...
            "no Neko call hook registered (use interpreter or VM mode for handlers)",
        )
    })?;
...
/// Suppress `print()` output (used by `neko bench`).
...
pub fn error_value(code: u32, kind: impl Into<String>, message: impl Into<String>, span: Span) -> ValueRef {
    Value::Error(NekoErrorValue::new(code, kind, message, span)).ref_cell()
}

pub fn error_from_runtime(err: &RuntimeError) -> ValueRef {
    Value::Error(err.to_neko_error_value()).ref_cell()
}

pub fn value_to_error(val: &Value) -> Option<NekoErrorValue> {
    match val {
...
pub type NativeFn = Rc<dyn Fn(&[ValueRef], Span) -> NekoResult<ValueRef>>;
...
pub fn apply_binop(op: BinOp, left: &Value, right: &Value, span: Span) -> NekoResult<Value> {
    match op {
...
fn add_values(left: &Value, right: &Value, span: Span) -> NekoResult<Value> {
    match (left, right) {
...
fn sub_values(left: &Value, right: &Value, span: Span) -> NekoResult<Value> {
    match (left, right) {
...
fn mul_values(left: &Value, right: &Value, span: Span) -> NekoResult<Value> {
    match (left, right) {
...
fn div_values(left: &Value, right: &Value, span: Span) -> NekoResult<Value> {
    match (left, right) {
...
fn floor_div_values(left: &Value, right: &Value, span: Span) -> NekoResult<Value> {
    match (left, right) {
...
fn mod_values(left: &Value, right: &Value, span: Span) -> NekoResult<Value> {
    match (left, right) {
...
pub fn apply_unaryop(op: UnaryOp, val: &Value, span: Span) -> NekoResult<Value> {
    match op {
...
fn cmp_op<F>(left: &Value, right: &Value, f: F, span: Span) -> NekoResult<Value>
where
    F: Fn(Ordering) -> bool,
{
...
fn compare_values(left: &Value, right: &Value, span: Span) -> NekoResult<Ordering> {
    match (left, right) {
...
fn to_float(v: &Value, span: Span) -> NekoResult<f64> {
    match v {
...
/// Mod arithmetic stress loop — matches `benchmark_math.py` / `math_bench.neko`.
...
pub fn builtin_super_boom_factorial(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    let n = expect_int_arg(args, span, "super_boom_factorial")?;
    Ok(super_boom_factorial_compute(n).ref_cell())
}

pub fn builtin_print_super_boom_factorial(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    let n = expect_int_arg(args, span, "print_super_boom_factorial")?;
    print_super_boom_factorial_int(n);
    Ok(Value::Nil.ref_cell())
}

pub fn builtin_super_boom_math(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    let n = expect_int_arg(args, span, "super_boom_math")?;
    Ok(Value::Int(super_boom_math_compute(n)).ref_cell())
}

pub fn builtin_print_super_boom_math(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    let n = expect_int_arg(args, span, "print_super_boom_math")?;
    print_super_boom_math_int(n);
    Ok(Value::Nil.ref_cell())
}

fn expect_int_arg(args: &[ValueRef], span: Span, name: &str) -> NekoResult<i64> {
    if args.len() != 1 {
...
pub fn builtin_print(args: &[ValueRef], _span: Span) -> NekoResult<ValueRef> {
    if quiet_output() {
...
pub fn builtin_len(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.len() != 1 {
        return Err(RuntimeError::at(
            span,
            1001,
...
pub fn builtin_type(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.len() != 1 {
        return Err(RuntimeError::at(
            span,
            1002,
...
pub fn builtin_has_trait(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.len() != 2 {
        return Err(RuntimeError::at(
            span,
            1002,
...
pub fn builtin_assert(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.is_empty() {
...
pub fn builtin_error(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    match args.len() {
        1 => {
...
pub fn builtin_is_error(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.len() != 1 {
        return Err(RuntimeError::at(
            span,
            codes::E1001_BUILTIN_ARITY,
...
pub fn builtin_now_ms(_args: &[ValueRef], _span: Span) -> NekoResult<ValueRef> {
    let ms = monotonic_start().elapsed().as_secs_f64() * 1000.0;
    Ok(Value::Int(ms as i64).ref_cell())
}

pub fn builtin_now_us(_args: &[ValueRef], _span: Span) -> NekoResult<ValueRef> {
    Ok(Value::Int(monotonic_start().elapsed().as_micros() as i64).ref_cell())
}

fn expect_size(args: &[ValueRef], span: Span, name: &str) -> NekoResult<usize> {
    if args.len() != 1 {
        return Err(RuntimeError::at(
            span,
            1007,
...
pub fn builtin_make_int_array(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    let n = expect_size(args, span, "make_int_array")?;
    Ok(Value::IntArray(vec![0; n]).ref_cell())
}

pub fn builtin_make_random_int_array(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn builtin_super_booster_sort(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.len() != 1 {
        return Err(RuntimeError::at(
            span,
            1009,
...
pub fn builtin_is_sorted(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.len() != 1 {
        return Err(RuntimeError::at(
            span,
            1010,
...
/// Fingerprint of the builtin table. Bumped `.nekobc` caches must match this
