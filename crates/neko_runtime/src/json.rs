...
fn value_to_json(v: &Value, span: Span) -> NekoResult<JsonValue> {
    match v {
...
fn get_object_key(map: &HashMap<String, ValueRef>, key: &str, span: Span) -> NekoResult<ValueRef> {
    map.get(key)
        .map(Rc::clone)
        .ok_or_else(|| type_err(span, format!("json_get: key '{key}' not found")))
}

fn get_at_path_rc(val: &ValueRef, tokens: &[PathToken], span: Span) -> NekoResult<ValueRef> {
    if tokens.is_empty() {
...
fn set_at_leaf(target: &mut Value, token: &PathToken, new_val: ValueRef, span: Span) -> NekoResult<()> {
    match (target, token) {
...
fn set_at_path(target: &ValueRef, tokens: &[PathToken], new_val: ValueRef, span: Span) -> NekoResult<()> {
    if tokens.is_empty() {
...
fn deep_merge_into(target: &mut Value, source: &Value, span: Span) -> NekoResult<()> {
    match (target, source) {
...
// ---------------------------------------------------------------------------
// Direct parse (serde visitor → Neko Value, no JsonValue tree)
// ---------------------------------------------------------------------------

struct NekoJsonVisitor;

struct NekoJsonSeed;

impl<'de> DeserializeSeed<'de> for NekoJsonSeed {
    type Value = Value;

    fn deserialize<D: Deserializer<'de>>(self, deserializer: D) -> Result<Self::Value, D::Error> {
        deserializer.deserialize_any(NekoJsonVisitor)
    }
}

impl<'de> Visitor<'de> for NekoJsonVisitor {
    type Value = Value;
    ...
    fn visit_seq<A: SeqAccess<'de>>(self, mut seq: A) -> Result<Self::Value, A::Error> {
        ...
        while let Some(v) = seq.next_element_seed(NekoJsonSeed)? {
            out.push(v.ref_cell());
        }
        Ok(Value::Array(out))
    }
    ...
    fn visit_map<A: MapAccess<'de>>(self, mut map: A) -> Result<Self::Value, A::Error> {
        ...
        while let Some(key) = map.next_key::<String>()? {
            let v = map.next_value_seed(NekoJsonSeed)?;
...
fn parse_json_text(text: &str, span: Span) -> NekoResult<Value> {
    let mut de = serde_json::Deserializer::from_slice(text.as_bytes());
    NekoJsonSeed.deserialize(&mut de).map_err(|e| {
        RuntimeError::at(
            span,
...
// ---------------------------------------------------------------------------
// Direct stringify (Neko Value → JSON text, no JsonValue tree)
// ---------------------------------------------------------------------------
...
fn stringify_value(v: &Value, out: &mut String, span: Span) -> NekoResult<()> {
    match v {
...
fn stringify_value_to_string(v: &Value, span: Span) -> NekoResult<String> {
...
pub fn json_parse(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn json_stringify(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    arity(args, 1, "json_stringify", span)?;
    let out = stringify_value_to_string(&args[0].borrow(), span)?;
    Ok(Value::String(out).ref_cell())
}

fn json_stringify_pretty(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn json_valid(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn json_type(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    arity(args, 1, "json_type", span)?;
    Ok(Value::String(json_type_name(&args[0].borrow()).to_string()).ref_cell())
}

fn json_is_json(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    arity(args, 1, "json_is_json", span)?;
    Ok(Value::Bool(is_json_value(&args[0].borrow())).ref_cell())
}

fn json_keys(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn json_has(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn json_get(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn json_set(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn json_merge(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    arity(args, 2, "json_merge", span)?;
    deep_merge_into(&mut args[0].borrow_mut(), &args[1].borrow(), span)?;
    Ok(Value::Nil.ref_cell())
}

fn json_clone(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    arity(args, 1, "json_clone", span)?;
    Ok(clone_json_value(&args[0].borrow()).ref_cell())
}

fn json_equal(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn json_array_len(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn json_object_len(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
mod tests {
    ...
    fn round_trip_object() {
        let span = dummy_span();
        let mut map = HashMap::new();
        map.insert("name".into(), Value::String("Neko".into()).ref_cell());
        ...
        assert_eq!(s, r#"{"name":"Neko","version":1}"#);
