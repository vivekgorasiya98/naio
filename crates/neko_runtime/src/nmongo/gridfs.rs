...
use super::types::{bson_doc_to_neko_ref, neko_to_bson};
...
pub fn nmongo_gridfs_upload(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    ...
    .map(|id| super::types::bson_to_neko(id).ref_cell())
    .or_else(|e| Ok(error_from_runtime(&e)))
}

pub fn nmongo_gridfs_download(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nmongo_gridfs_delete(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nmongo_gridfs_list(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    ...
    with_client(client_id, "nmongo_gridfs_list", span, |client| {
        block_on(async move {
            ...
            while let Some(doc) = cursor.next().await {
                ...
                files.push(bson_doc_to_neko_ref(&bson_doc));
            }
            Ok(files)
        })
    })
