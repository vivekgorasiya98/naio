...
pub fn nmongo_list_indexes(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    ...
    with_client(client_id, "nmongo_list_indexes", span, |client| {
        block_on(async move {
            ...
            while let Some(model) = cursor.next().await {
                ...
                indexes.push(bson_doc_to_neko_ref(&doc));
            }
            Ok(indexes)
        })
    })
...
pub fn nmongo_drop_index(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
