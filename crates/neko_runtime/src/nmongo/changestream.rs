...
use super::types::bson_doc_to_neko_ref;
...
pub fn nmongo_watch(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    ...
    let pipeline = if args.len() >= 4 {
        match &*args[3].borrow() {
            Value::Array(_) => {
                ...
                for s in &arr {
                    stages.push(super::types::neko_to_bson(s, span)?);
                }
                stages
            }
            _ => Vec::new(),
...
pub fn nmongo_watch_next(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    ...
    if let Some(doc) = queue.first() {
        let event = bson_doc_to_neko_ref(doc);
        queue.remove(0);
        return Ok(event);
    }
...
pub fn nmongo_watch_close(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
