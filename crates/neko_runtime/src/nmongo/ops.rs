...
use super::types::{bson_to_neko, neko_to_bson};
...
pub fn parse_find_options(
    ...
    if let Some(map) = opts {
        ...
        if let Some(v) = map.get("sort") {
            out.sort = Some(neko_to_bson(v, span)?);
        }
        if let Some(v) = map.get("projection") {
            out.projection = Some(neko_to_bson(v, span)?);
        }
    }
    Ok(out)
}
...
pub fn insert_result_to_neko(result: mongodb::results::InsertOneResult) -> HashMap<String, ValueRef> {
    let mut map = HashMap::new();
    map.insert(
        "inserted_id".to_string(),
        bson_to_neko(result.inserted_id).ref_cell(),
    );
    map
}

pub fn insert_many_result_to_neko(
    ...
    let ids: Vec<ValueRef> = result
        .inserted_ids
        .into_iter()
        .map(|(_, id)| bson_to_neko(id).ref_cell())
        .collect();
    map.insert("inserted_ids".to_string(), Value::Array(ids).ref_cell());
    map
}

pub fn update_result_to_neko(result: mongodb::results::UpdateResult) -> HashMap<String, ValueRef> {
    let mut map = HashMap::new();
    map.insert(
        "matched".to_string(),
    ...
    if let Some(id) = result.upserted_id {
        map.insert("upserted_id".to_string(), bson_to_neko(id).ref_cell());
    }
    map
}

pub fn delete_result_to_neko(result: mongodb::results::DeleteResult) -> HashMap<String, ValueRef> {
    let mut map = HashMap::new();
    map.insert(
