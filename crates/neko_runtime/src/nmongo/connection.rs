...
use super::common::*;
use super::handles::{alloc_client, remove_client, with_client};
...
pub fn nmongo_connect(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nmongo_connect_uri(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    arity(args, 1, "nmongo_connect_uri", span)?;
    let uri = string_arg(args, 0, "nmongo_connect_uri", span)?;

    let client_opts = block_on(async move { ClientOptions::parse(&uri).await }).map_err(|e| {
        RuntimeError::at(
            span,
            codes::E1921_NMONGO_ERROR,
...
pub fn nmongo_close(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nmongo_ping(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nmongo_list_databases(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
