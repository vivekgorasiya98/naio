...
pub fn neko_to_bound(val: &Value, span: Span) -> Result<BoundValue, crate::RuntimeError> {
    match val {
        ...
        Value::Object(map) => {
            ...
            for (k, v) in map {
                json_map.insert(k.clone(), neko_to_json(&*v.borrow())?);
            }
...
fn neko_to_json(val: &Value) -> Result<JsonValue, crate::RuntimeError> {
    match val {
        ...
        Value::Array(items) => {
            ...
            for item in items {
                arr.push(neko_to_json(&*item.borrow())?);
            }
...
pub fn pg_to_neko(row: &Row, i: usize) -> Value {
    ...
    if let Ok(v) = row.try_get::<_, Option<JsonValue>>(i) {
        return v.map(|j| json_to_neko(&j)).unwrap_or(Value::Nil);
    }
...
fn json_to_neko(j: &JsonValue) -> Value {
    match j {
        ...
        JsonValue::Array(items) => {
            Value::Array(items.iter().map(|v| json_to_neko(v).ref_cell()).collect())
        }
        JsonValue::Object(map) => {
            let mut out = HashMap::new();
            for (k, v) in map {
                out.insert(k.clone(), json_to_neko(v).ref_cell());
            }
            Value::Object(out)
        }
    }
