...
impl MemSnapshot {
    fn current() -> Self {
        ...
        let budget = neko_tensor::pool::memory_budget();
        let tracked = neko_tensor::pool::memory_used();
        Self {
...
fn int_field(
    ...
    match &*val.borrow() {
        ...
        other => Err(RuntimeError::at(
            ...
            format!("neko_mem_* expected int field '{key}', got {}", other.type_name()),
        )),
    }
}
...
fn neko_mem_stats(_args: &[ValueRef], _span: Span) -> NekoResult<ValueRef> {
    Ok(Value::Object(collect_stats()).ref_cell())
}

fn neko_mem_format(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    let snap = if args.is_empty() {
        MemSnapshot::current()
    } else if args.len() == 1 {
        snapshot_from_arg(&args[0], "neko_mem_format", span)?
    } else {
        return Err(RuntimeError::at(
            span,
            codes::E1974_NML_TYPE,
            format!(
                "neko_mem_format expects 0 or 1 argument(s), got {}",
                args.len()
            ),
        )
        .into());
...
fn neko_mem_diff(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.len() != 2 {
        return Err(RuntimeError::at(
            span,
            codes::E1974_NML_TYPE,
            format!("neko_mem_diff expects 2 argument(s), got {}", args.len()),
        )
        .into());
    }
    let before = snapshot_from_arg(&args[0], "neko_mem_diff", span)?;
    let after = snapshot_from_arg(&args[1], "neko_mem_diff", span)?;
...
pub fn builtins() -> Vec<(&'static str, NativeFn)> {
    vec![
        ("neko_mem_stats", Rc::new(neko_mem_stats)),
        ("neko_mem_format", Rc::new(neko_mem_format)),
        ("neko_mem_diff", Rc::new(neko_mem_diff)),
    ]
}
