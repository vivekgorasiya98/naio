...
use neko_ml::loss::LossKind;
use neko_ml::optimizer::OptimizerKind;
use neko_ml::trainer::Trainer;
use neko_ml::tuning::{EarlyStopping, GridSearch, RandomSearch, SearchResult, trainer_from_params};
...
pub fn nml_trainer(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nml_train_epoch(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nml_plot_training(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nml_eval(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nml_save(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    ...
    with_handle(id, "nml_save", span, |h| match h {
        NmlHandle::Model(m) => {
            neko_ml::checkpoint::save_model(std::path::Path::new(&path), m)
                .map_err(|e| e.to_string())?;
            Ok(())
        }
        NmlHandle::Trainer(t) => {
            neko_ml::checkpoint::save_model(std::path::Path::new(&path), &t.model)
                .map_err(|e| e.to_string())?;
            Ok(())
        }
...
pub fn nml_load(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    arity(args, 1, "nml_load", span)?;
    let path = string_arg(args, 0, "nml_load", span)?;
    let model = neko_ml::checkpoint::load_model(
        std::path::Path::new(&path),
        neko_tensor::global_device(),
    )
    .map_err(|e| RuntimeError::at(span, codes::E1971_NML_ERROR, e.to_string()))?;
    Ok(ok_handle(alloc_handle(NmlHandle::Model(model))))
}

pub fn nml_grid_search(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nml_random_search(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nml_early_stopping(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
pub fn nml_memory_budget(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    if args.is_empty() {
    ...
    neko_tensor::pool::set_memory_budget(bytes);
    Ok(Value::Nil.ref_cell())
}

pub fn nml_explain(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
    ...
    let importances = with_handle(model_id, "nml_explain", span, |h| match h {
        NmlHandle::Model(m) => neko_ml::feature_importance(m, &x).map_err(|e| e.to_string()),
        NmlHandle::Trainer(t) => {
            neko_ml::feature_importance(&t.model, &x).map_err(|e| e.to_string())
        }
        _ => Err("expected model or trainer".into()),
    })?;
...
pub fn nml_backward_step(args: &[ValueRef], span: Span) -> NekoResult<ValueRef> {
...
fn extract_xy(
    ...
    y_id: u64,
    span: Span,
) -> Result<(neko_tensor::Tensor, neko_tensor::Tensor), String> {
