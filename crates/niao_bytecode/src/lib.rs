...
impl From<niao_ir::IrError> for CompileError {
    fn from(e: niao_ir::IrError) -> Self {
        CompileError::Ir(e)
    }
}

/// Increment when bytecode layout or semantics change so sidecar caches recompile.
pub const BYTECODE_CACHE_VERSION: u32 = 11;
...
impl BytecodeModule {
    /// Serialize to compact binary `.niaobc` wire format.
    pub fn serialize(&self) -> Vec<u8> {
        wire::encode(self)
    }

    /// Load from binary `.niaobc` wire format, with legacy JSON fallback.
    ...
    pub fn is_cache_valid(&self) -> bool {
        self.cache_version == BYTECODE_CACHE_VERSION
            && self.builtin_fingerprint == niao_runtime::builtin_fingerprint()
            && self.call_targets_are_current()
    }

    fn call_targets_are_current(&self) -> bool {
        let builtins = niao_runtime::builtin_names();
...
pub fn compile_to_bytecode(program: &Program) -> Result<BytecodeModule, CompileError> {
    ...
    // Single source of truth: every builtin registered in niao_runtime
    // (core + dsa) becomes a callable target.
    for builtin in niao_runtime::builtin_names() {
    ...
    Ok(BytecodeModule {
        ...
        builtin_fingerprint: niao_runtime::builtin_fingerprint(),
        source_path: None,
        classes: ir.classes,
        traits: ir.traits,
        field_names,
...
fn is_self_recursive(f: &niao_ir::IrFunction) -> bool {
...
fn lower_instr(
    ...
    Ok(match instr {
        ...
        IrInstr::Binary(niao_ast::BinOp::Add) => OpCode::Add,
        IrInstr::Binary(niao_ast::BinOp::Sub) => OpCode::Sub,
        IrInstr::Binary(niao_ast::BinOp::Mul) => OpCode::Mul,
        IrInstr::Binary(niao_ast::BinOp::Div) => OpCode::Div,
        IrInstr::Binary(niao_ast::BinOp::FloorDiv) => OpCode::FloorDiv,
        IrInstr::Binary(niao_ast::BinOp::Mod) => OpCode::Mod,
        IrInstr::Binary(niao_ast::BinOp::Eq) => OpCode::Eq,
        IrInstr::Binary(niao_ast::BinOp::Ne) => OpCode::Ne,
        IrInstr::Binary(niao_ast::BinOp::Lt) => OpCode::Lt,
        IrInstr::Binary(niao_ast::BinOp::Gt) => OpCode::Gt,
        IrInstr::Binary(niao_ast::BinOp::Le) => OpCode::Le,
        IrInstr::Binary(niao_ast::BinOp::Ge) => OpCode::Ge,
        IrInstr::Binary(niao_ast::BinOp::And) => OpCode::And,
        IrInstr::Binary(niao_ast::BinOp::Or) => OpCode::Or,
        IrInstr::Unary(niao_ast::UnaryOp::Not) => OpCode::Not,
        IrInstr::Unary(niao_ast::UnaryOp::Neg) => OpCode::Neg,
