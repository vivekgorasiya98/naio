...
impl From<neko_parser::ParseError> for InterpreterError {
...
impl Interpreter {
    ...
    /// Run entry file and keep the Neko call hook active (for long-running servers).
    ...
    fn install_call_hook(&mut self) {
        ...
        *GLOBAL_INTERP.lock().unwrap() = Some(ptr as usize);
        set_neko_fn_resolver(Some(Arc::new(|name| {
        ...
        set_neko_call_hook(Some(Arc::new(|callee, args, span| {
            let guard = GLOBAL_INTERP.lock().unwrap();
    ...
    fn clear_call_hook(&mut self) {
        set_neko_call_hook(None);
        set_neko_fn_resolver(None);
        *GLOBAL_INTERP.lock().unwrap() = None;
    }

    /// Invoke a Neko callable from native code (HTTP server handlers).
    ...
    fn load_module(&mut self, imp: &ImportStmt) -> Result<(), InterpreterError> {
        ...
        if neko_runtime::native_module_paths().contains(&import_path.trim_matches('"')) {
            if let Some(export) = neko_runtime::native_module_export_name(import_path) {
        ...
        let source = fs::read_to_string(&resolved).map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                ...
                let hint = if matches!(path, "nmongo" | "std/nmongo") {
                    " (nmongo is not enabled in this neko build)"
                } else {
                    ""
                };
    ...
    fn resolve_module_path(&self, import_path: &str) -> PathBuf {
        ...
        if path.ends_with(".neko") {
            self.base_dir.join(path)
        } else {
            self.base_dir.join(format!("{path}.neko"))
        }
    }
        ...
        match stmt {
            ...
            Stmt::Throw { value, span } => {
                ...
                let err = match &*val.borrow() {
                    ...
                    other => RuntimeError::thrown(NekoErrorValue::from_message(
                        other.to_string(),
                        *span,
                    )),
                };
...
fn resolve_stdlib_path(stdlib: &Path, import_path: &str) -> Option<PathBuf> {
    ...
    let direct = stdlib.join(format!("{rel}.neko"));
    if direct.is_file() {
        return Some(direct);
    }
    let lib_file = stdlib.join(rel).join("lib.neko");
    if lib_file.is_file() {
...
mod tests {
    ...
    #[test]
    fn runs_hello() {
        let src = include_str!("../../../examples/hello.neko");
        run(src).unwrap();
    }

    #[test]
    fn runs_fibonacci() {
        let src = include_str!("../../../examples/fibonacci.neko");
        run(src).unwrap();
    }

    #[test]
    fn runs_super_booster_sort() {
        let src = include_str!("../../../examples/super_booster_sort.neko");
        run(src).unwrap();
    }
}
