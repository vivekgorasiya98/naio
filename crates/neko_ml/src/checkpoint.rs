...
pub fn save_model(path: &Path, model: &Sequential) -> TensorResult<()> {
    ...
    let mut f = File::create(path).map_err(|e| neko_tensor::TensorError::Io(e.to_string()))?;
    f.write_all(&buf)
        .map_err(|e| neko_tensor::TensorError::Io(e.to_string()))?;
    Ok(())
}

pub fn load_model(path: &Path, device: Device) -> TensorResult<Sequential> {
    let mut f = File::open(path).map_err(|e| neko_tensor::TensorError::Io(e.to_string()))?;
    let mut buf = Vec::new();
    f.read_to_end(&mut buf)
        .map_err(|e| neko_tensor::TensorError::Io(e.to_string()))?;
    let mut pos = 0;
    if buf.len() < 12 || &buf[0..4] != MAGIC {
        return Err(neko_tensor::TensorError::Io("invalid .nml file".into()));
    }
    pos += 4;
