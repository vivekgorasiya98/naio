use crate::catalog::{standard_libs, LibSpec, NEKO_TOOLCHAIN_VERSION};
...
/// Locate the Neko source tree that contains `neko_libs/`.
pub fn find_source_root() -> Option<PathBuf> {
    if let Ok(dir) = env::var("NEKO_SOURCE") {
...
pub fn find_source_root_or_err() -> PkgResult<PathBuf> {
    find_source_root().ok_or_else(|| {
        PkgError::Message(
            "Neko source root not found. Set NEKO_SOURCE, run from the Neko repo, or pass --source <path>".into(),
        )
    })
}

pub fn source_libs_dir(source_root: &Path) -> PathBuf {
    source_root.join("neko_libs")
}

fn is_source_root(path: &Path) -> bool {
    path.join("neko_libs").is_dir() && path.join("Cargo.toml").is_file()
}
...
pub fn toolchain_version_from_source(_source_root: &Path) -> String {
    NEKO_TOOLCHAIN_VERSION.to_string()
}

/// Fresh `neko` / `nm` from `cargo build --release` under the source tree.
