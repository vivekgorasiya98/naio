use neko_ast::*;
use neko_lexer::{Token, TokenKind};
use std::collections::HashSet;

pub use neko_errors::ParseError;
...
pub fn parse(source: &str) -> Result<Program, ParseError> {
    let tokens = neko_lexer::lex(source).map_err(ParseError::from)?;
...
"#;
    ...
    fn parses_loops_example() {
        let src = include_str!("../../../examples/loops.neko");
        parse(src).unwrap();
    }
    ...
    fn parses_structs_example() {
        let src = include_str!("../../../examples/structs.neko");
        parse(src).unwrap();
    }
    ...
    fn parses_oop_basics() {
        let src = include_str!("../../../examples/oop_basics.neko");
        parse(src).unwrap();
    }
}
