...
use neko_ast::TopLevel;
use neko_parser::parse;
use neko_runtime::ahiru::{
    self, clear_vm_pool, clear_vm_serve, finalize_vm_handlers, install_vm_pool, response_from_neko,
    set_vm_serve_active, VmPoolDispatchFn,
};
use neko_runtime::{quiet_output, set_ahiru_serve_options, set_quiet_output, apply_ahiru_cli_port};
use neko_vm::ahiru_pool::VmHandlerPool;
use neko_vm::call_bridge::{clear_thread_vm_hook, run_with_handler_hook};
use neko_vm::Vm;
use std::fs;
...
fn run_interp_serve(
    ...
    let serve_result = if result.is_ok() {
        neko_runtime::start_ahiru_pending_server().map_err(|e| e.to_string())
    } else {
        Ok(())
    };
...
fn run_vm_serve(entry: &Path, base: &Path, project: &Path) -> Result<(), String> {
    ...
    let fields_to_args = Arc::new(|fields: &std::collections::HashMap<String, String>| {
        ahiru::ctx_bridge::fields_to_neko(fields)
    });
    let handler_index = Arc::new(|id: u64| ahiru::handler_vm_index(id));
    let to_response = Arc::new(|val: &neko_runtime::Value| response_from_neko(val));
...
fn collect_watch_files(project: &Path) -> Vec<PathBuf> {
    let mut out = Vec::new();
    for name in ["ahiru.config.toml", "neko.config"] {
    ...
    if src.is_dir() {
        walk_neko_files(&src, &mut out);
    }
    out
}

fn walk_neko_files(dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            walk_neko_files(&path, out);
        } else if path.extension().is_some_and(|e| e == "neko") {
            out.push(path);
        }
    }
}
...
fn resolve_entry(project: &Path, file: Option<&Path>) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    let neko_config = project.join("neko.config");
    if neko_config.is_file() {
        let text = fs::read_to_string(&neko_config)?;
    ...
    let default = project.join("src/main.neko");
    if default.is_file() {
        return Ok(default);
    }
    Err("could not find entry .neko file".into())
}

fn locate_stdlib(project: &Path) -> Option<PathBuf> {
    let mut candidates: Vec<PathBuf> = Vec::new();
    if let Ok(env) = std::env::var("NEKO_STDLIB") {
